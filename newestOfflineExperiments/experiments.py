    ...
    def runHiddenLayersExperiment(self, experiment_name, usecase, replay_size, num_hidden_layers):
        ...
        for i, train_batch in enumerate(dataset):
            ...
            if i == 1:
                ...
                cl_model.model.compile(optimizer=tf.keras.optimizers.RMSprop(learning_rate=0.00005),
                                       loss='sparse_categorical_crossentropy', metrics=['accuracy'])
                cl_model.head.compile(optimizer=tf.keras.optimizers.RMSprop(learning_rate=0.001),
                                      loss='sparse_categorical_crossentropy', metrics=['accuracy'])
